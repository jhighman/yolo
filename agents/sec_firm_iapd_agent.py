...
class SECFirmIAPDAgent:
    ...
    def get_firm_details(self, organization_crd: str, reference_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Get detailed information about a firm from SEC IAPD.
        ...
        url = f"{IAPD_CONFIG['base_search_url']}/{organization_crd}"
        self.logger.info("Starting SEC IAPD detailed firm search", extra=params)
        ...
        try:
            ...
            # Validate and parse response
            data = self.validate_response(response, params)
            self.logger.debug("Raw API response", extra={**params, "raw_response": data})

            if data["hits"]["hits"]:
                content = data["hits"]["hits"][0].get("_source", {})
                self.logger.info(
                    "Detailed data fetched successfully",
                    extra={**params, "content_keys": list(content.keys())}
                )
                return content
            else:
                self.logger.warning("No details found for the specified CRD", extra=params)
                return {}
    ...
    def normalize_response(self, response: Dict[str, Any]) -> Dict[str, Any]:
        """Normalize the SEC response to match our schema."""
        source = response.get('source', {})
        return {
            'organization_crd': source.get('org_pk', ''),
            'firm_name': source.get('org_name', ''),
            'crd_number': source.get('org_pk', ''),
            'firm_url': f"https://adviserinfo.sec.gov/firm/summary/{source.get('org_pk', '')}",
            'sec_number': source.get('sec_number', ''),
