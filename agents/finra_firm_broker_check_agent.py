...
class FinraFirmBrokerCheckAgent:
    ...
    @rate_limit
    def get_firm_details(self, organization_crd: str, reference_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Get detailed information about a firm from FINRA BrokerCheck.
        
        Args:
            organization_crd: The firm's CRD number.
            reference_id: Optional reference ID for tracking the request.
        
        Returns:
            Dict containing firm details.
        """
        params = {
        ...
        url = f'https://api.brokercheck.finra.org/search/firm/{organization_crd}'
        self.logger.info("Starting FINRA BrokerCheck detailed firm search", extra=params)
        ...
        try:
            ...
            self.logger.debug("Outgoing request details", extra={**params, "request": request_details})
            ...
            self.logger.debug("Response details", extra={**params, "response": response_details})

            if response.status_code == 200:
                raw_data = response.json()
                # Log raw response data at debug level
                self.logger.debug("Raw API response", extra={**params, "raw_response": raw_data})
                ...
                if "hits" in raw_data and raw_data["hits"]["hits"]:
                    ...
                    try:
                        ...
                        self.logger.error("Failed to parse content JSON", 
                                        extra={**params, "error": str(e), "content": content_str})
                        return {}
                else:
                    self.logger.warning("No hits found in detailed response", extra=params)
                    return {}
            else:
                self.logger.error("Unexpected status code", 
                                extra={**params, "status_code": response.status_code})
                return {}
    ...
    def normalize_response(self, response: Dict[str, Any]) -> Dict[str, Any]:
        """Normalize the FINRA response to match our schema."""
        source = response.get('source', {})
        return {
            'organization_crd': source.get('org_source_id', ''),
            'firm_name': source.get('org_name', ''),
            'firm_other_names': source.get('firm_other_names', []),
            'firm_ia_scope': source.get('firm_ia_scope', ''),
            'firm_ia_disclosure_fl': source.get('firm_ia_disclosure_fl', ''),
            'firm_branches_count': source.get('firm_branches_count', 0),
            'firm_ia_address_details': source.get('firm_ia_address_details', {})
        }
    ...
    def get_firm_details_menu():
        if not last_results:
            ...
            for i, firm in enumerate(last_results, 1):
                print(f"{i}. {firm['firm_name']} (CRD: {firm['organization_crd']})")
            ...
            try:
                ...
                if 0 <= idx < len(last_results):
                    crd = last_results[idx]['organization_crd']
                else:
                    crd = choice
            except ValueError:
                crd = choice
