...
def setup_logging():
    ...
    log_file = os.path.join(log_dir, 'sec_api_test.log')
    
    logger = logging.getLogger('sec_api_test')
...
def run_tests():
    ...
    logger.info("Starting SEC IAPD API tests with real API")
    
    # Create an agent with real API calls
    agent = SECFirmIAPDAgent(use_mock=False)
    logger.info("Created SEC IAPD agent with real API")
    ...
    for i, entity in enumerate(SAMPLE_ENTITIES, 1):
        ...
        try:
            ...
            try:
                ...
                from agents.sec_firm_iapd_agent import IAPD_CONFIG
                url = IAPD_CONFIG["firm_search_url"]
                params = {**IAPD_CONFIG["default_params"], "query": entity['name']}
            ...
            result = agent.search_firm_by_crd(entity['organizationCRD'], employee_number)
