#!/usr/bin/env python3
"""
Test script for the FINRA BrokerCheck agent with CRD 10740.
...
def test_search_entity():
    ...
    try:
        ...
        if result:
            ...
            print(f"Entity data: {json.dumps(result, indent=2)[:1000]}...")  # Show first 1000 chars
            return True
        else:
...
def test_get_firm_details():
    ...
    try:
        ...
        if details:
            ...
            # Print some key information from the details
            if isinstance(details, dict):
                firm_name = details.get("firmName", "Unknown")
                firm_status = details.get("firmStatus", "Unknown")
                print(f"Firm Name: {firm_name}")
                print(f"Firm Status: {firm_status}")
                print(f"Details preview: {json.dumps(details, indent=2)[:1000]}...")  # Show first 1000 chars
            return True
        else:
...
def main():
    """Run all tests."""
    print("Starting FINRA BrokerCheck Agent tests for CRD 10740...")
