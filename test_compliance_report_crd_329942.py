#!/usr/bin/env python3
"""
Script to generate a full compliance report for a firm with CRD 329942.
This script tests the case where a firm should be flagged as not having a current registration.
"""
...
def main():
    """Generate a full compliance report for firm with CRD 329942 with additional diagnostics."""
    # Create facade
    ...
    if firm_details:
        ...
        if report:
            ...
            if alerts:
                ...
                # Verify that we have the expected alert for no current registration
                has_no_registration_alert = any(
                    alert.alert_type == "NoActiveRegistration" for alert in alerts
                )
                if has_no_registration_alert:
                    logger.info("✅ TEST PASSED: Alert for Status - no current registration found")
                else:
                    logger.error("❌ TEST FAILED: Expected alert for no current registration not found")
            else:
                logger.error("❌ TEST FAILED: No alerts found, expected alert for no current registration")
            ...
            if has_no_registration_alert_in_report:
                logger.info("✅ Report contains NoActiveRegistration alert")
            else:
